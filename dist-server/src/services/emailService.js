...
class EmailService {
    async scanInbox(userId) {
        // 1. Get tokens from Supabase
        const { data: profile } = await dbService_1.supabase
            .from('profiles')
            .select('gmail_tokens')
            .eq('id', userId)
            .single();
        if (!profile?.gmail_tokens)
            return;
        const oauth2Client = new googleapis_1.google.auth.OAuth2(process.env.GMAIL_CLIENT_ID, process.env.GMAIL_CLIENT_SECRET);
        ...
        for (const msg of res.data.messages) {
            ...
            const analysis = await aiService_1.aiService.analyzeEmail(body);
    ...
    async createDraft(userId, to, subject, body) {
        const { data: profile } = await dbService_1.supabase
            .from('profiles')
            .select('gmail_tokens')
            .eq('id', userId)
