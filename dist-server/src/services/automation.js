...
exports.resumeVariantService = exports.platformHealthService = exports.autoApplyService = exports.jobQueueService = void 0;
...
exports.jobQueueService = {
    ...
    async logToSupabase(job, status, error) {
        try {
            await axios_1.default.post('/api/sheet', {
                jobId: job.id,
                jobTitle: job.title,
                company: job.company,
...
exports.autoApplyService = {
    ...
    enable(config) {
        ...
        exports.jobQueueService.emitEvent('autoapply.enabled', { minScore: this.minScore, maxPerHour: this.maxPerHour });
    },
    disable() {
        this.enabled = false;
        exports.jobQueueService.emitEvent('autoapply.disabled', {});
    },
    shouldAutoApply(job) {
        if (!this.enabled)
            return false;
        ...
        if (this.appliedThisHour >= this.maxPerHour) {
            exports.jobQueueService.emitEvent('autoapply.limit_reached', { hourLimit: this.maxPerHour });
            return false;
        }
...
exports.platformHealthService = {
    health: {},
    async trackSuccess(platform, duration) {
    ...
    async trackFailure(platform, duration) {
        if (!this.health[platform]) {
            this.health[platform] = { success: 0, total: 0, avgTime: 0, lastCheck: '', status: 'healthy' };
        }
        this.health[platform].total++;
...
exports.resumeVariantService = {
    ...
    selectBestForJob(job) {
        if (this.variants.length === 0)
            return null;
        const relevantVariants = this.variants.filter(v => v.type === job.jobType || v.type === 'general');
        return relevantVariants.sort((a, b) => b.usageCount - a.usageCount)[0] || this.variants[0];
    },
    recordUsage(variantId) {
