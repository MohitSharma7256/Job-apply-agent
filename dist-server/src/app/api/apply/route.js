"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.runtime = void 0;
exports.POST = POST;
exports.GET = GET;
const server_1 = require("next/server");
const dbService_1 = require("../../../services/dbService");
const socketServer_1 = require("../../../server/socketServer");
exports.runtime = 'nodejs';
async function POST(request) {
    try {
        const body = await request.json();
        const { jobId, userId, resumeVariantId } = body;
        const { data: job } = await dbService_1.supabase
            .from('jobs')
            .select('*')
            .eq('id', jobId)
            .single();
        if (!job) {
            return server_1.NextResponse.json({ error: 'Job not found' }, { status: 404 });
        }
        const { data: application } = await dbService_1.supabase
            .from('applications')
            .insert({
            jobId: job.id,
            status: 'processing',
        })
            .select()
            .single();
        const applicationId = application?.id;
        (0, socketServer_1.emitToUser)(userId, 'job:applying', {
            jobId: job.id,
            step: 'login',
            title: job.title,
            company: job.company,
        });
        setTimeout(() => {
            (0, socketServer_1.emitToUser)(userId, 'job:applying', {
                jobId: job.id,
                step: 'filling',
            });
        }, 2000);
        setTimeout(() => {
            (0, socketServer_1.emitToUser)(userId, 'job:applying', {
                jobId: job.id,
                step: 'submitting',
            });
        }, 4000);
        setTimeout(async () => {
            await dbService_1.supabase
                .from('applications')
                .update({
                status: 'success',
                appliedAt: new Date().toISOString(),
            })
                .eq('id', applicationId);
            await dbService_1.supabase
                .from('jobs')
                .update({ status: 'applied' })
                .eq('id', jobId);
            (0, socketServer_1.emitToUser)(userId, 'job:applied', {
                jobId: job.id,
                applicationId,
                timestamp: new Date().toISOString(),
                title: job.title,
                company: job.company,
            });
        }, 6000);
        return server_1.NextResponse.json({
            success: true,
            applicationId,
            message: 'Application submitted',
        });
    }
    catch (error) {
        console.error('Apply error:', error);
        return server_1.NextResponse.json({ error: error.message }, { status: 500 });
    }
}
async function GET(request) {
    const { searchParams } = new URL(request.url);
    const userId = searchParams.get('userId');
    if (!userId) {
        return server_1.NextResponse.json({ error: 'User ID required' }, { status: 400 });
    }
    const { data: applications, error } = await dbService_1.supabase
        .from('applications')
        .select(`
      *,
      job:jobs(title, company, location, platform, matchScore)
    `)
        .eq('userId', userId)
        .order('createdAt', { ascending: false });
    return server_1.NextResponse.json({
        success: !error,
        applications: applications || [],
    });
}
